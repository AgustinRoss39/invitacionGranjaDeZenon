...
function fitNameTitles() {
  ...
  fitTextToContainer($("#guest-name"), 72, 30);
}
...
function updateCountdown() {
  ...
  if (now >= end) {
    finishCountdown("¡Gracias por venir a la granja! 🐮");
    return;
  }

  if (now >= start) {
    finishCountdown("¡Hoy es el gran día! 🚜", "¡La fiesta ya empezó!");
    return;
  }
  ...
  if (days === 0) {
    ...
    countdownStatus.textContent = `Faltan ${days} días para la fiesta.`;
  }
}
